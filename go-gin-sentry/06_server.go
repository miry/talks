...
//
// Usage:
//  SENTRY_DSN=<url> go run 06_server.go
...
func main() {
	...
	r.GET("/error", func(c *gin.Context) {
		...
		err = errors.Wrap(err, "Access Denied") // HL
		l.Warn().Err(err).Str("endpoint", "error_log").Msg("Wrapped error") // HL
